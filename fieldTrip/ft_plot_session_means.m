...
for ff=1:length(sd.freq_labels)
  ...
  for ll=1:nsites 
    ...
    if strcmp(sd.freq_labels{ff}, 'hgamma') || strcmp(sd.freq_labels{ff}, 'gamma'), clim = [0 12];
    else; clim = [-10 1]; end
    hold on;
...
%Gonna plot the min/max responses per electrode
    ...
    [onsett_p, onsett_h] = ranksum(sigt(1:pd_n), sigt((end-et_n+1):end));
    ...
    fprintf('PD - mean peak Z:%f   mean peak time:%f  mean_onset_time: %f\n', ...
        mean(pd_resp), mean(pd_peakt), nanmean(sigt(1:pd_n)));
    fprintf('ET - mean peak Z:%f   mean peak time:%f  mean_onset_time: %f\n', ...
        mean(et_resp), mean(et_peakt), nanmean(sigt((end-et_n+1):end)));
    fprintf('Peak Times (Rank Sum) - h:%f  p:%f\n', peakt_h, peakt_p);
    fprintf('Onset Times (Rank Sum) - h:%f  p:%f\n', onsett_h, onsett_p);
  end

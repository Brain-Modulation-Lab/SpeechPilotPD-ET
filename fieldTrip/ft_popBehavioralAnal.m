...
personalDefaults;
setDirectories;
load([savedDataDir filesep 'Population' filesep 'PD_populationBehavior.mat']);
nSessionsPD = length(sessionBehavior);
ET = load([savedDataDir filesep 'Population' filesep 'ET_populationBehavior.mat']);
...
allLatsM = NaN*zeros(120, nSessions);
...
for ii=1:nSessions
    ...
    allLatsM(1:ntrials,ii) = sessionBehavior(ii).SpLatency;
    allDurM(1:ntrials,ii) = sessionBehavior(ii).SpDuration;
    %allSNRM(:,ii) = 20*log10(sessionBehavior(ii).snrVoice); %SNR db as in SLP matlab book
end

%% Reaction times
PDlat = cat(1, sessionBehavior(PDsession==1).SpLatency);
ETlat = cat(1, sessionBehavior(PDsession==0).SpLatency);
figure; 
edges = [0:0.1:5 Inf];
...
%% Speech Duration
PDdur = cat(1, sessionBehavior(PDsession==1).SpDuration);
ETdur = cat(1, sessionBehavior(PDsession==0).SpDuration);
figure; 
edges = [0:0.1:1.5 Inf];

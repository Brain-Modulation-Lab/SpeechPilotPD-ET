...
for subject_id = 1:length(subject_list)
    ...
    adir = ['Anatomy' fs 'Freesurfer' fs '*' fs 'Electrode_Locations' 'MNI_on_cortex' fs 'CortElecLocL.mat'];
    % direct to CortElecLoc file
    elec_dir{subject_id} = dir([dataDir fs subject_name fs adir]);
    if isempty(elec_dir{subject_id})
        elec_dir{subject_id} = dir([dataDir subject_name fs 'Anatomy' fs 'Freesurfer' fs 'Electrode_Locations' fs 'MNI_on_cortex' fs 'CortElecLocL.mat']);
        if isempty(elec_dir{subject_id})

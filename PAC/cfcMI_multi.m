function [mi,amp_ph,zmi]=cfcMI_multi(ph,amp,nbin,niter)
...
zmi=zeros(ac,pc);
% 
Psurr=zeros(nbin,niter*ac);

for np=1:pc
[~,whichbin(:,np)]=histc(ph(:,np),bins);
end
samp=zeros(size(amp,1),niter*ac);
u=1:niter:niter*ac;
for na=1:ac
samp(:,u(na):u(na)+niter-1)=repmat(amp(:,na),1,niter);
end
shift=randperm(s,niter*ac)';
for n=1:niter*ac
...
Psurr=reshape(Psurr,nbin,niter,ac);

Psurr=bsxfun(@rdivide,Psurr,sum(Psurr,1));
    
        %compute modulation index
        nullmi=(maxE+sum(Psurr.*log2(Psurr),1))/maxE;
        
        %z-score actual mi
        zmi(:,np)=(mi(:,np)-squeeze(mean(nullmi,2)))./squeeze(std(nullmi,0,2));
end
